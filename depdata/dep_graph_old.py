...
class DepGraph:
    ...
    def cve_check(self, source:str):
        ''' target is NodeId
        
        '''
        # get attribute nodes
        target_node_list = self.addvalue_dict[source]
    ...
    def get_cve_releases(self,):
        return {node_id: data for node_id, data in self.nodes.items() if data['labels'] == ":AddedValue" and self.cve_check(node_id)}
